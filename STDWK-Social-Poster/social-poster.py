...
# Define push_to_netlify before any other functions
...
def create_scheduler_csv(data_rows):
    ...
    csv_path = os.path.join(OUTPUT_FOLDER, 'InstagramSchedulerPrep.csv')
...
def create_updated_events_csv(data_rows):
    ...
    csv_path = os.path.join(OUTPUT_FOLDER, 'events_update.csv')
...
def main():
    try:
        ...
        if push_to_netlify():
            ...
            print(f"Scheduler CSV created in {OUTPUT_FOLDER}/InstagramSchedulerPrep.csv")
            print(f"Updated events CSV created in {OUTPUT_FOLDER}/events_update.csv")
        else:
